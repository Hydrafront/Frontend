...
const LockToken: React.FC<LockTokenProps> = ({ setEnabled }) => {
  const [_token, _setToken] = useState(
    "0x599777F03f97ce4c039689b197fF6542fd990fca"
  );
  ...
  const createLock = async () => {
    ...
    try {
      ...
      await writeContractAsync({
        ...
        args: [_token, parseEther(_amount.toString()), _tmpLockDuration],
      });
    } catch (e) {
      console.log(e);
    }
    ...
    <div className="w-full md:w-1/2 mb-10 md:mb-0 px-8">
      ...
      <div className="mb-10 mt-5">
        ...
        <CustomSelect
          ...
          label="Select lock duration"
          value={_lockDuration}
          onChange={(e) => _setLockDuration(e as string)}
        />
      </div>
      <GradientButton className="w-full rounded-lg mb-4" onClick={createLock}>
        Create Lock
      </GradientButton>
      <button
        className="px-5 py-2 w-full rounded-lg bg-borderColor mb-8"
